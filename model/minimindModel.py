...
class MindModel(nn.Module):
    def __init__(self, config: MindConfig):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.layers = nn.ModuleList([MindBlock(l, config) for l in range(self.num_hidden_layers)])
...
class MindForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MindConfig = None):
        self.config = config or MindConfig()
        super().__init__(self.config)
        self.model = MindModel(self.config)
